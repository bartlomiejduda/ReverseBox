...
class TranslationEntry:
    ...
    text_transform_function: Optional[Callable[[bytes], bytes]] = None
...
class TranslationTextHandler(FileHandler):
    ...
    def export_all_text(
        ...
        for translation_entry in self.translation_memory:
            ...
            if translation_entry.text_transform_function:
                text_entry_bytes = translation_entry.text_transform_function(
                    text_entry_bytes
                )
            text_entry_str: str = text_entry_bytes.decode(encoding)

            output_po_file.append(
                polib.POEntry(comment=text_entry_key, msgid=text_entry_str, msgstr="")
            )
        try:
    ...
    def import_all_text(self, input_po_file_path, encoding: str = "utf8") -> bool:
        ...
        for po_entry in input_po_file:
            print(po_entry.msgid, po_entry.msgstr)  # TODO
