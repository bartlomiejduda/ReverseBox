...
from reversebox.image.swizzling.swizzle_wii_u import swizzle_wii_u, unswizzle_wii_u

# fmt: off


@pytest.mark.imagetest
def test_wii_u_unswizzle_and_swizzle_monkey_sample_type0():
    swizzled_file_path = os.path.join(
        os.path.dirname(__file__), "image_files/swizzle_wii_u_monkey_sample_type0.bin"
    )
    ...
    unswizzled_file_data = unswizzle_wii_u(img_width, img_height, image_format=0x00000031, tile_mode=4, swizzle_type=851968, pitch=64,
                                           input_data=swizzled_file_data)
    ...
    reswizzled_file_data = swizzle_wii_u(img_width, img_height, image_format=0x00000031, tile_mode=4, swizzle_type=851968, pitch=64,
                                         input_data=swizzled_file_data)
